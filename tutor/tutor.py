...
class ExampleApp(QtWidgets.QMainWindow, Ui_mainWindow):
            ...
            for host in hosts:
                try:
                    ...
                    if 'runningtrainer.conf\n' in stdout.readlines():
                        stdin, stdout, stderr = ssh.exec_command('cat /etc/trainerIBOS/runningtrainer.conf')
    ...
    def ShowsResult(self):
        ...
        for host in hosts:
            ...
            if 'result.csv\n' in stdout.readlines():
                ...
                scp.get('/etc/trainerIBOS/result.csv', '/etc/trainerIBOStutor/result.csv')
                with open('/etc/trainerIBOStutor/result.csv', 'r') as fileResult:
    ...
    def ToGoExam(self):
        ...
        ListResult = []
        amountRow = 0
        with open('/etc/trainerIBOStutor/exam.conf', 'w') as examFile:
        ...
        for host in hosts:
            try:
                ...
                scp = SCPClient(ssh.get_transport())
                scp.put('/etc/trainerIBOStutor/exam.conf', '/etc/trainerIBOS/exam.conf')
                ssh.close()
            except:
                continue
        os.remove('/etc/trainerIBOStutor/exam.conf')
